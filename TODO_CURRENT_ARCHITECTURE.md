...
- Config manifest refactor completed: `config.yaml` now orients around declarative sections (cameras, detection, storage, system, dashboards, outputs) with inline help comments for every tunable field, nested notification/video routing, renamed storage subdirs (`snap_subdir`, `gif_subdir`, `video_subdir`), and manifest lists for dashboard/output modules. `ConfigSnapshot` builders ingest the manifests while remaining hot-reload compatible.
- Media routing and processing are decoupled: detection routing selects per-event outputs (text/snap/gif/video) while `media_artifacts` governs clip generation.
...
- Multi-camera pipelines are first-class: `config.yaml` accepts an array of `cameras[]`, ConfigService emits per-camera module configs, and the orchestrator instantiates USB/RTSP/simulator inputs plus downstream motion/YOLO consumers for every defined feed.
...
- `core.config`: manifest-aware extraction helpers (dashboards/outputs module lists, nested notification/media sections), new `SnapshotOutputSettings`/`GifOutputSettings`/`VideoNotificationSettings`, renamed storage subdir fields, and builder fallbacks so manifests + legacy hot reload payloads both work.
...
- `tests/unit/test_config.py`: asserts S3/websocket/resilience/db-logger builders plus manifest/detection/media routing behavior.
- `tests/unit/test_storage_retention.py`: extended to cover remote reconciliation hooks.
- `tests/conftest.py`: sample config updated to the new manifest layout (nested notification/media sections, renamed storage subdirs).
...
- Config UX: every parameter in `config/config.yaml` now carries inline documentation, reinforcing best practices for operators and third-party module authors.
