...
class SecurityEventManager:
    ...
    def __init__(
        ...
        self.on_motion: Callable[[Optional[str]], None] = self._default_motion_handler
        self.on_person: Callable[[Optional[str]], None] = self._default_person_handler
    ...
    def _default_motion_handler(self, event_file: Optional[str]) -> None:
        """Default handler for motion events."""
        self._stats['motion_events'] += 1
        self.logger.info(f"Motion detected: {event_file or 'processing...'}")
    
    def _default_person_handler(self, event_file: Optional[str]) -> None:
    ...
    def _cleanup_all_events(self) -> int:
        ...
        try:
            ...
            if files_failed:
                self.logger.error(f"Failed to delete {len(files_failed)} files: {', '.join(files_failed[:5])}")
    ...
    def list_recordings(self, limit: Optional[int] = None) -> List[str]:
        """
    ...
    def get_recording(self, event_name: str) -> Optional[str]:
        """
