...
class BotConfig:
    ...
    # Multi-user settings
    notification_chat_id: int | None = None  # Dedicated chat for alerts
    ...
    write_timeout: int = (
        60  # Telegram write timeout (seconds)            # Silent rejection in groups
    )
...
class SecurityTelegramBot:
    ...
    def _is_private_chat(self, chat_id: int | None = None) -> bool:
    ...
    def _get_notification_chat_id(self) -> int | None:
    ...
    def _handle_motion_event(self, event_file: str) -> None:
        ...
        self.logger.debug(
            f"Motion event received - file: {event_file}, exists: {Path(event_file).exists()}"
        )

        event = NotificationEvent(
            message="👀 Motion detected", event_file=event_file, event_type="motion"
        )
    ...
    def _handle_person_event(self, event_file: str) -> None:
        ...
        # File is guaranteed to exist at this point
        self.logger.debug(
            f"Person event received - file: {event_file}, exists: {Path(event_file).exists()}"
        )

        event = NotificationEvent(
            message="🚨 Person detected!", event_file=event_file, event_type="person"
        )
        self._queue_notification(event)

    def _handle_disconnect_event(self) -> None:
        """Handle capture disconnection events."""
        event = NotificationEvent(message="⚠️ Capture disconnected!", event_type="disconnect")
        self._queue_notification(event)
    ...
    def _update_chat_id(self, update: Update) -> None:
        ...
        if not self.chat_id and update.effective_chat:
            ...
            if self._is_group_context(update) and not self.config.notification_chat_id:
                self.logger.warning(
                    "Using group chat for notifications - consider setting notification_chat_id"
                )
    ...
    async def _send_notification(
        self, context: ContextTypes.DEFAULT_TYPE, event: NotificationEvent
    ) -> None:
        ...
        try:
            ...
            should_send_media = (
                event.event_file
                and Path(event.event_file).exists()
                and (
                    (event.event_type == "person" and self.config.gif_for_person)
                    or (event.event_type == "motion" and self.config.gif_for_motion)
                )
            )
            ...
            if hasattr(self.event_manager, "event_logger"):
                from analytics import EventType, SecurityEvent

                self.event_manager.event_logger.log_event(
                    SecurityEvent(
    ...
    async def _send_text_notification(
        ...
        await context.bot.send_message(chat_id=self.chat_id, text=caption, parse_mode="HTML")
    ...
    async def _unauthorized_response(self, update: Update) -> None:
        ...
        self.logger.warning(
            f"Unauthorized access attempt from user {user_id} ({update.effective_user.full_name})"
        )
        ...
        if self._is_group_context(update) and self.config.silent_unauthorized:
            ...
            await update.message.reply_text(
                "🚫 Access denied.", reply_to_message_id=update.message.message_id
            )
        else:
    ...
    def _validate_numeric_arg(
        ...
        try:
            ...
            return (
                None,
                f"❌ Invalid {arg_name} (must be a {'number' if not as_int else 'whole number'})",
            )
    ...
    async def cmd_whitelist_add(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if user_id not in self._user_whitelist:
                ...
                self.logger.info(
                    f"Superuser {update.effective_user.id} added user {user_id} to whitelist"
                )
            ...
            self.logger.warning(
                f"Superuser {update.effective_user.id} provided invalid user ID: {context.args[0]}"
            )
    ...
    async def cmd_whitelist_remove(
        ...
        try:
            ...
            if user_id in self._user_whitelist:
                ...
                self.logger.info(
                    f"Superuser {update.effective_user.id} removed user {user_id} from whitelist"
                )
    ...
    async def cmd_recordings(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            for recording in today_recordings:
                today_buttons.append(
                    InlineKeyboardButton(
                        recording["button_text"], callback_data=recording["callback_data"]
                    )
                )
        ...
        if older_recordings:
            ...
            date_range = (
                f"{oldest_date}" if oldest_date == newest_date else f"{oldest_date}-{newest_date}"
            )
    ...
    async def cmd_test_notification(
        ...
        test_event = NotificationEvent(message="🧪 Test notification", event_type="test")
    ...
    async def cmd_timeline(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            await update.message.reply_photo(
                photo=io.BytesIO(plot_data), caption=caption, parse_mode="HTML"
            )
    ...
    async def start_bot(self) -> None:
        ...
        try:
            ...
            self.app.job_queue.run_repeating(
                self._process_notification_queue, interval=1.0, first=0.5
            )
