...
class SecuritySystem:
    ...
    def __init__(
        ...
        self.gif_executor = ThreadPoolExecutor(
            max_workers=self.gif_worker_threads, thread_name_prefix="SecurityGifProcessor"
        )
        ...
        try:
            ...
            if not model_path.exists():
                ...
                try:
                    ...
                    _temp_model = YOLO(
                        "yolov8n.pt"
                    )  # Downloads to cache, variable prefixed with _ to indicate intentionally unused
                    ...
                    try:
                        ...
                        if cached.exists():
                            ...
                            self.logger.warning("Using model from ultralytics cache")
                            model_path = cached
            ...
            dummy_frame = np.zeros(
                (self.yolo_warmup_size, self.yolo_warmup_size, 3), dtype=np.uint8
            )
    ...
    def _optimize_batch_size(
        self, frame_shape: tuple[int, int, int] = None, memory_reserve: float = None
    ) -> int:
        """
        ...
        try:
            ...
            self.logger.debug(
                f"GPU memory optimization: {free_gb:.1f}GB free, batch_size={optimal_batch_size}"
            )
    ...
    def _add_motion_overlay(
        self, frame: np.ndarray, fg_mask: np.ndarray, motion_score: int
    ) -> None:
        ...
        try:
            ...
            cv2.rectangle(
                frame, (x, y), (x + panel_w, y + panel_h), accent, max(1, int(scale_factor))
            )
            ...
            cv2.putText(
                frame,
                status,
                (dot_x + dot_radius + int(5 * scale_factor), status_y),
            ...
            cv2.rectangle(
                frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_height), (60, 60, 60), -1
            )
            ...
            if progress > 0:
                cv2.rectangle(
                    frame,
                    (bar_x, bar_y),
                    (bar_x + int(bar_w * progress), bar_y + bar_height),
                    accent,
                    -1,
                )
    ...
    def _handle_motion_event(self) -> None:
        ...
        try:
            ...
            has_person, frames, cached_detections, frames_processed = (
                self._collect_and_detect_person()
            )
    ...
    def _process_complete_event(self, event_data: dict) -> str | None:
        """
        ...
        try:
            for frame in self._record_frames_streaming():
                ...
                if len(mini_batch) >= self.batch_size:
                    person_detected, bbox, batch_detections = self._detect_person_in_batch(
                        mini_batch
                    )
                    ...
                    if has_person:
                        self.logger.debug(
                            f"Person detected, early exit after {frames_processed} frames"
                        )
            ...
            self.logger.debug(
                f"Collected {len(frames)} frames, processed {frames_processed} with YOLO, person={has_person}"
            )
        ...
        try:
            ...
            for result in results:
                ...
                if result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for box, cls_id in zip(boxes, classes, strict=False):
                        if int(cls_id) == 0:  # Person class
                            person_found = True
                            area = (box[2] - box[0]) * (box[3] - box[1])
                            if largest_bbox is None or area > (
                                largest_bbox[2] - largest_bbox[0]
        ...
        try:
            if has_person:
                ...
                if cached_detections and len(cached_detections) > 0:
                    ...
                    self.logger.debug(
                        f"Using {cached_count} cached detections, processing {len(frames) - cached_count} remaining frames"
                    )

                    for frame, detection in zip(
                        frames[:cached_count], cached_detections, strict=False
                    ):
                ...
                if remaining_frames:
                    self.logger.debug(
                        f"Processing {len(remaining_frames)} remaining frames with YOLO"
                    )
                            ...
                            if result.boxes is not None and len(result.boxes) > 0:
                                ...
                                for box, cls_id in zip(boxes, classes, strict=False):
                                    if int(cls_id) == 0:  # Person class
                                        x1, y1, x2, y2 = box.astype(int)
                                        cv2.rectangle(
                                            frame_copy, (x1, y1), (x2, y2), (0, 0, 255), 3
                                        )
    ...
    def _generate_gif(self, frames: list[np.ndarray], output_path: Path, fps: int) -> bool:
        ...
        try:
            ...
            # Generate GIF
            imageio.mimsave(
                str(output_path), gif_frames, format="GIF", fps=fps, loop=0, quantizer="nq"
            )

            if output_path.exists():
                file_size = output_path.stat().st_size / 1024
                self.logger.info(
                    f"GIF created: {output_path.name} ({file_size:.1f}KB, {len(gif_frames)} frames)"
                )
                return True
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "person_cooldown_active": time.time() - self.last_person_detection
            < self.person_cooldown_seconds,
